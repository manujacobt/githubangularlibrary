...
function validateJwtFields(payload, options) {
    ...
    if (payload.exp !== undefined) {
        if (payload.exp + (options.graceSeconds ?? 0) < Date.now() / 1000) {
            throw new error_js_1.JwtExpiredError(`Token expired at ${new Date(payload.exp * 1000).toISOString()}`);
        }
    }
    // Check not before
    if (payload.nbf !== undefined) {
        if (payload.nbf - (options.graceSeconds ?? 0) > Date.now() / 1000) {
            throw new error_js_1.JwtNotBeforeError(`Token can't be used before ${new Date(payload.nbf * 1000).toISOString()}`);
        }
    }
    // Check JWT issuer
    if (options.issuer !== null) {
        ...
        (0, assert_js_1.assertStringArrayContainsString)("Issuer", payload.iss, options.issuer);
    }
    // Check audience
    if (options.audience !== null) {
        ...
        (0, assert_js_1.assertStringArraysOverlap)("Audience", payload.aud, options.audience);
    }
    // Check scope
    if (options.scope != null) {
        (0, assert_js_1.assertStringArraysOverlap)("Scope", payload.scope?.split(" "), options.scope);
    }
}
