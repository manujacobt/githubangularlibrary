...
import { SimpleJwksCache, isJwk, isJwks, fetchJwk, } from "./jwk.js";
...
import { JwtInvalidSignatureError, JwtInvalidClaimError, ParameterValidationError, KidNotFoundInJwksError, } from "./error.js";
/**
 ...
 * @returns void
 */
function verifySignatureAgainstJwk(header, headerB64, payload, payloadB64, signatureB64, jwk, jwkToKeyObjectTransformer = transformJwkToKeyObject) {
    // Check JWK use
 ...
 */
export async function verifyJwt(jwt, jwksUri, options, jwkFetcher, jwkToKeyObjectTransformer) {
    return verifyDecomposedJwt(decomposeJwt(jwt), jwksUri, options, jwkFetcher, jwkToKeyObjectTransformer);
}
/**
 ...
 * @returns Promise that resolves to the payload of the JWT––if the JWT is valid, otherwise the promise rejects
 */
async function verifyDecomposedJwt(decomposedJwt, jwksUri, options, jwkFetcher = fetchJwk, jwkToKeyObjectTransformer) {
 ...
 */
export function verifyJwtSync(jwt, jwkOrJwks, options, jwkToKeyObjectTransformer) {
    return verifyDecomposedJwtSync(decomposeJwt(jwt), jwkOrJwks, options, jwkToKeyObjectTransformer);
}
/**
...
function verifyDecomposedJwtSync(decomposedJwt, jwkOrJwks, options, jwkToKeyObjectTransformer) {
    ...
    if (options.customJwtCheck) {
        ...
        if (res !== undefined && res instanceof Promise) {
            throw new ParameterValidationError("Custom JWT checks must be synchronous but a promise was returned");
        }
    }
    return payload;
}
...
export class JwtRsaVerifierBase {
    ...
    getVerifyParameters(jwt, verifyProperties) {
        ...
        assertStringArrayContainsString("Issuer", decomposedJwt.payload.iss, this.expectedIssuers);
    ...
    withJwksUri(config) {
        ...
        return {
            jwksUri: new URL(join(issuerUri.pathname, "/.well-known/jwks.json"), config.issuer).href,
            ...config,
        };
    }
}
 ...
 */
export const transformJwkToKeyObject = (jwk) => createPublicKey({
...
export class KeyObjectCache {
    constructor(jwkToKeyObjectTransformer = transformJwkToKeyObject) {
