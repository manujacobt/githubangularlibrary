...
import { JwtExpiredError, JwtNotBeforeError, JwtParseError, ParameterValidationError, } from "./error.js";
/**
...
export function validateJwtFields(payload, options) {
    ...
    if (payload.nbf !== undefined) {
        if (payload.nbf - (options.graceSeconds ?? 0) > Date.now() / 1000) {
            throw new JwtNotBeforeError(`Token can't be used before ${new Date(payload.nbf * 1000).toISOString()}`);
        }
    }
    // Check JWT issuer
    ...
    if (options.scope != null) {
        assertStringArraysOverlap("Scope", payload.scope?.split(" "), options.scope);
    }
}
